    ...
    public partial class PurchaseOrders : Form
    {
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            foreach (DataGridViewRow item in this.dataGridView2.SelectedRows)
            {
                ...
                con.Open();
                string query = "DELETE FROM TBL_PO_ITEMS WHERE POID= " + POID + ";";
                SqlCommand cmd = new SqlCommand(query, con);
                cmd.ExecuteNonQuery();
                query = "DELETE FROM TBL_PURCHASE_ORDER WHERE POID= " + POID + ";";
        ...
        {
            purchaseOrder = this.dataGridView2.CurrentRow.Cells[0].Value.ToString();
            if (vpo == null)
            {
        ...
        {
            string query = " SELECT * FROM TBL_PURCHASE_ORDER WHERE created_date >= '" + dateTimePicker1.Value.Date.ToString("yyyyMMdd") + " ' AND created_date <= '" +
