...
namespace Account_Payable1
{
    public partial class ViewPurchaseOrder : Form
    {
        ...
        {
            // TODO: This line of code loads data into the 'accountsPayableDataSet.TBL_PO_ITEMS' table. You can move, or remove it, as needed.
            this.tBL_PO_ITEMSTableAdapter.Fill(this.accountsPayableDataSet.TBL_PO_ITEMS);
            ...
            String query = "SELECT NUM_vendorid, FT_total FROM TBL_PURCHASE_ORDER WHERE NUM_POID = " + POIDtxt.Text.Trim() + ";";
            SqlDataAdapter sda1 = new SqlDataAdapter(query, con);
            DataTable dtbl2 = new DataTable();
            sda1.Fill(dtbl2);
            textBox1.Text = Convert.ToString(dtbl2.Rows[0][0]);
            totallbl.Text = Convert.ToString(dtbl2.Rows[0][1]);

            query = "SELECT * FROM TBL_VENDOR INNER JOIN TBL_PURCHASE_ORDER ON NUM_Vendor_ID=NUM_vendorid WHERE NUM_POID = " + POIDtxt.Text.Trim() + ";";
        ...
        {
            string query = " SELECT TBL_PO_ITEMS.NUM_itemID, TBL_PO_ITEMS.NUM_quantity, TBL_SC_ITEMS.CHR_item_name, TBL_SC_ITEMS.FT_price " +
"FROM TBL_PO_ITEMS INNER JOIN TBL_SC_ITEMS ON TBL_PO_ITEMS.NUM_itemID = TBL_SC_ITEMS.NUM_itemID WHERE TBL_PO_ITEMS.NUM_POID = '"
+ POIDtxt.Text.Trim() + "'";
            SqlDataAdapter sda = new SqlDataAdapter(query, con);
            DataTable dtbl = new DataTable();
            sda.Fill(dtbl);
